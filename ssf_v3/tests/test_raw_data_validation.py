...
`analysis/support_reference.py` (the immutable reference), function by function, on
...
sys.path.insert(0, os.path.join(REPOSITORY_ROOT, "analysis"))
...
def test_build_fine_table() -> None:
    ...
    conditioned_without_extras = conditioned.drop(columns=["discount"])
