...
4. **Cuándo medir dinámica.** Estacionalidad y tendencia solo se miden después de reparar el soporte (P9): con n pequeño, cualquier pendiente es ruido. El criterio de «hay señal» no es un umbral fijo sino la referencia binomial propia de cada serie (P1): **φ** = variación observada de la tasa mes a mes / variación que tendría que haber solo por muestreo si la tasa fuera constante (la binomial la da exacta: con p = 0,8 y n = 100, ±4 pp al mes sin que pase nada). φ ≈ 1: la serie oscila lo que la moneda predice, no hay nada que modelar y el promedio es la mejor técnica. φ > 1: hay un motor (estación, tendencia, régimen) y ahí una serie temporal tiene algo que capturar. Es lo que decide dónde merece la pena una técnica compleja.
