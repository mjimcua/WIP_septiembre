...
- **Dónde vive**: `fu_id`, `fu_key`; una fila de `fact_fu`; la referencia inmutable `fact_fu` (columnas se_pp_max / moe_pp_max / moe_usd_max) (soporte y error binomial peor-caso por FU).
...
- **Ejemplo**: las series `EU|1|0|0|0|A|web` (dormant, n=15), `EU|0|1|0|0|A|web` (softcancel, n=12) y `EU|0|0|1|0|A|web` (no_instalado, n=10) forman el pool L1 `EU|SIG=negativo|A|web` con n=37, que supera el suelo.
...
- **L1 · agrupar por signo**: las series con timevarying activas del mismo signo (neg/pos) se juntan en un pool `SIG=negativo` / `SIG=positivo`. Los motivos se pierden, el signo se conserva.
...
- **Dónde vive**: tabla `mandatory_only_cost` (`celda`, `mes`, `err_plano_pp`, `err_seg_pp`, `ahorro_usd`).
...
- **Qué es**: en el ensamblaje (fase 3), cada fila futura del pipeline necesita una tasa. Se busca en orden y se anota de dónde salió:
  ...
  2. si la serie no tiene estimación (ruta solo_futuro: futuro sin historia), la **media de su celda mandatory** → `"celda"`;
...
- **Qué es**: el mes en curso tiene el pipeline completo (se sabe qué vence) pero el resultado incompleto (aún no han renovado todos). Regla sellada (16-ago-2026): se le asigna el rol `proyeccion` sea cual sea su etiqueta, es el **primer mes a proyectar**, nunca test ni train, y se borran (NaN) las renovaciones y readquisiciones ya contabilizadas en él y en cualquier mes de proyección, para que el futuro parezca no empezado.
...
### Universo (`universo`)
- `normal` o `serie_temporal`, según `flag_time_series`. El universo `serie_temporal` está reservado a un tratamiento propio (por eso `ts_revenue_col` sigue declarado en Config); hoy solo se etiqueta.

### Cobertura (`cobertura`) y ruta (`tratamiento`)
- **Cobertura**: qué roles tiene una serie, ordenados y unidos con `_` (`projection_test_train`, `proyeccion`, `train`…).
- **Ruta**: qué se hace con la serie, deducido de la cobertura: `predecible` (historia + futuro: se estima su tasa), `solo_futuro` (futuro sin historia: la cascada de fallback le da la tasa de su celda), `solo_historia` (sin futuro: nada que predecir; se etiqueta, no se borra — P4).
- Test solo no es historia: `projection_test` es `solo_futuro`.
...
- **Dos niveles**: por **forecast unit** (`fact_fu` (columnas se_pp_max / moe_pp_max / moe_usd_max): cada mes con su n, a p = 0,5 como peor caso antes de estimar nada) y por **forecast series** (ficha: con su tasa y su n mensual típico).
...
- **Signo**: cada timevarying declara si empuja hacia churn (`negative`: softcancel, dormant, no_instalado) o hacia renovar (`positive`: autorenew). Una serie con alguna negativa activa es de signo `negativo`; con positiva, `positivo`; con ambas, `neg+pos`; sin ninguna, neutra.
...
- **id_estimacion**: el patrón del pariente elegido por la escalera (`EU|SIG=negativo|A|*`); una serie con soporte propio es su propio id. Es la clave por la que se diagnostica la dinámica, se hace el backtest, se elige la técnica y se calculan las bandas. Todas las series que casan el patrón calculan su serie mensual; solo las que lo eligieron lo reciben.
- **Niveles de riesgo** (`nivel_riesgo`): `A_propio` (peldaño 0, ≥ 12 meses) · `A2_propio_corto` (peldaño 0, < 12 meses) · `B_prestado` (peldaño 1-2) · `C_lejano` (peldaño ≥ 3, o neutra que no alcanza el suelo) · `D_sin_historia` · `S_signo_bajo_suelo` (con signo, celda × signo bajo suelo: ruidosa pero cualificada) · `M_signo_mixto` (ambos signos: sola) · `N_sin_impacto` (sin proyección) · `T_universo_ts`.
...
- **gate** (`pool_reference`): `soporte` → `temporal` → `estacional` → `tendencia` → `apto_promedio`; primera puerta que se cierra. `estacional` 2 = firme (≥ 2 ciclos), 1 = tentativa. El perfil estacional se mide sobre la serie SIN tendencia.
...
## Añadidos de la versión final (septiembre 2026)

- **Calendario desde el mes en curso**: `proyeccion` (mes en curso y posteriores), `pendiente_cierre` (el anterior: ni verdad ni entrenamiento, pero se predice), `examen` (los `test_months` anteriores: solo evaluar), `entrenamiento` (el resto). El raw trae `dataset_role` con train/test/projection y se sobreescribe.
- **Tratamiento** (antes "ruta"): `predecible` (meses cerrados y algo que predecir), `solo_historia` (nada que predecir), `solo_futuro` (nada de qué aprender: cascada).
- **Signo**: `neutro`, `negativo`, `positivo`, `mixto`. Token en los ids de estimación: `SIG=negativo`.
- **Dos suelos**: `support_floor = 30` (quién puede prestar) y `own_rate_floor = 271` (quién va solo); entre los dos, `A3_propio_reforzado` (credibilidad z = n/(n+k)).
- **Benchmark de estacionalidad**: la decisión única sobre las series grandes neutras (`decision_estacionalidad`); φ, amplitud, consistencia, |z| de los meses extremos, prueba predictiva (T15 vs T3_ma3 a h=1 y h=6), materialidad en $. `pool_reference` traslada el veredicto a los pools.
- **Dos baterías**: `corto` (h=1: el mes siguiente con datos hasta el anterior) y `medio_largo` (h=6: con datos hasta seis meses antes); tope de horizonte 6.
- **Banda común**: los cuantiles del error del total (todos los pools sumados) por horizonte, en los meses de decisión (`decision_agg_bands`); sumada linealmente entre meses y combinada en cuadratura con la idiosincrática → banda total.
- **Origen de pipeline**: `real` (existe hoy), `proyectada` (reentrada de una renovación prevista), `simulada` (adquisición al ritmo histórico).
- **Maduración de señales**: composición por celda sin timevarying, foto mensual (`signal_snapshot`), maduración pendiente = final (12 meses cerrados) − actual, ajuste en $, `forecast_ajustado_usd`, alertas `nivel_anomalo` y `tendencia`.
- **Top movers**: `deterioro`, `mejora`, `senal_negativa`, `banda_ancha`, `sesgo_examen`, `precio`; en `top_movers`, ordenados por dinero dentro de cada tipo.
