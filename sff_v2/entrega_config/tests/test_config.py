...
EXPECTED_PHYSICAL_NAMES = {"forecast_units_raw_summary": "sff_fu_summary",
                           ...
                           "diagnostico_dinamica": "sff_diag_dinamica",
                           "forecast_final_seleccion": "sff_forecast_seleccion",
                           "rolling_next_month_resumen": "sff_rolling_nm_resumen",
...
def test_series_and_cell_columns() -> None:
    ...
    # the legacy Spanish aliases still answer while phases 1-4 use them
    RECORDER.check(configuration.grano_tasa == configuration.rate_series_columns
                   and configuration.grano_uplift == configuration.uplift_cell_columns
                   and configuration.medidas == configuration.core_measures
                   and configuration.medidas_declaradas == configuration.declared_measures,
                   "the legacy aliases return exactly the new properties")
...
def print_panel() -> None:
    """Print the per-block panel and the list of failures, if any.

    INPUT:   the module-level RECORDER, already populated.
    OUTPUT:  none.
    RULES:   one line per block with passed/total, then every failure in full. The
             panel is the part a reader keeps; the per-check lines above are the detail.
    EDGE CASES: a block with no checks would print 0/0; that means a test function was
             registered but ran nothing, which is itself worth seeing.
    CONSOLE: the panel.
    STEPS:
    ...
    for block_name in RECORDER.passed_by_block:
        passed_count = RECORDER.passed_by_block[block_name]
        failed_count = RECORDER.failed_by_block[block_name]
        block_total = passed_count + failed_count
        block_mark = "✓" if failed_count == 0 else "✗"
        print(f"  {block_mark} {block_name:<52} {passed_count:>3}/{block_total}")

    # [2] the totals
    total_checks = RECORDER.total_passed + RECORDER.total_failed
    print("─" * 74)
    print(f"  TOTAL {RECORDER.total_passed}/{total_checks} checks passed")

    # [3] what broke, in full, so the reader does not scroll back
    if RECORDER.failure_messages:
        print(f"\nFAILED CHECKS ({len(RECORDER.failure_messages)}):")
        for failure_message in RECORDER.failure_messages:
...
def main() -> int:
    ...
    print("Run this after every change to run/config.py, before the equivalence gate.")
    print("═" * 74)
