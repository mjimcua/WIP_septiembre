...
class Config:
    ...
    WHAT IT DOES NOT DO. It does not read the raw (the runner does), does not compute
    ...
    USAGE. `Config()` with no arguments is production. Tests and the golden run pass a
    smaller taxonomy and inject an engine. Field names are the public contract used by
    every phase, by the runners and by the regeneration prompts; they do not change.
    """
